...
const getWowPositionShortLabel = (positionId) => getWowPositionMeta(positionId)?.shortLabel || "";
...
export default function App() {
  ...
  const raidPositionStats = useMemo(() => {
    const stats = { 전체: raidEligibleRoster.length, tank: 0, dealer: 0, heal: 0 };
    raidEligibleRoster.forEach((member) => {
      normalizePreferredPositions(member.preferredPositions).forEach((positionId) => {
        stats[positionId] = (stats[positionId] || 0) + 1;
      });
  ...
  const handleQuickAddRaidMember = (memberId) => {
    const emptySlot = findNextEmptyRaidSlot(raidAssignments, { skipLockedSlot: true });
    if (!emptySlot) {
              ...
              <div className="flex overflow-x-auto gap-2 custom-scrollbar w-full items-center" style={{ scrollbarWidth: 'thin' }}>
                {wowPositionStats.orderedIds.map((positionId) => {
                  ...
                  return (
                    ...
                    >
                      {meta?.label}
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-black ${isSelected ? 'bg-black/10 text-black' : 'bg-black/30 text-gray-300'}`}>
                        {count}
                      </span>
                    </button>
                  );
                        ...
                        <td className="px-6 py-5">
                          <div className="flex flex-wrap gap-1.5">
                            {normalizePreferredPositions(member.preferredPositions).length > 0 ? normalizePreferredPositions(member.preferredPositions).map((positionId) => (
                              <span key={positionId} className="inline-flex items-center px-2 py-1 rounded-full bg-white text-black text-[15px] font-black border border-white/80 whitespace-nowrap leading-none">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {raidPositionStats.orderedIds.map((positionId) => {
                        ...
                        return (
                          ...
                          >
                            {meta?.label}
                            <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-black ${isSelected ? 'bg-black/10 text-black' : 'bg-black/30 text-gray-300'}`}>
                              {count}
                            </span>
                          </button>
                        );
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                                  {normalizePreferredPositions(member.preferredPositions).map((positionId) => (
                                    <span key={positionId} className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-white text-black text-[10px] font-black border border-white/80 whitespace-nowrap">
                                      {getWowPositionShortLabel(positionId)}
                                    </span>
                                  ))}
                                </div>
                ...
                <div className="flex flex-col gap-2 items-start lg:items-end">
                  ...
                  <div className="flex flex-wrap gap-2 justify-start lg:justify-end">
                    {WOW_POSITION_OPTIONS.filter((option) => option.id !== '전체').map((option) => (
                      <span key={option.id} className="inline-flex items-center px-2.5 py-1 rounded-full bg-white text-black text-[11px] font-black border border-white/80">
                        {option.shortLabel} <span className="ml-1">{raidAssignedPositionStats[option.id] || 0}</span>
                      </span>
                    ))}
