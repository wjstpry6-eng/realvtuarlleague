...
export default function App() {
  ...
  const [isCleaningGhosts, setIsCleaningGhosts] = useState(false); // ★ 유령 청소기 상태 추가
  ...
  useEffect(() => {
    ...
    const unsubPopup = onSnapshot(popupRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setSitePopup(data);
        if (data.isActive) {
          // 로컬 스토리지에 오늘 날짜+수정시간이 기록되어 있으면 안 띄움
  ...
  const renderAdminView = () => {
    ...
    const handleDeletePlayer = async (playerId, playerName) => {
      if (
        !user ||
        !window.confirm(
          `정말 [${playerName}] 선수를 명단에서 강제 삭제하시겠습니까?\n\n(주의: 이 선수가 참여한 경기 기록이 남아있다면 꼬일 수 있으니, 경기 기록이 없는 '유령 선수'만 삭제해주세요!)`
        )
      )
        return;
      try {
