  ...
  const handleSaveEditedMatch = async (e) => {
    ...
    try {
      for (const origResult of matchToEdit.originalMatch.results) {
        ...
        if (p) {
          // ★ 2. 경기 수정 시 오타/이름 변경으로 발생한 유령 선수 제거 로직 ★
          ...
          if (!isStillInThisMatch && !hasOtherMatches) {
            // 수정 후 명단에서 이름이 빠졌고, 다른 경기 기록도 없다면 완전 삭제 🗑️
            await deleteDoc(doc(db, "artifacts", appId, "public", "data", "players", p.id));
          } else {
            // 그대로 있거나 다른 경기에 참여했다면 예전 점수 차감
            ...
            <form onSubmit={handleSaveEditedMatch} className="space-y-6">
              ...
              {editMatchMode === "team" && (
                <div className="space-y-4 mt-4">
                  ...
                  <button type="button" onClick={() => setTeamResults([...teamResults, { id: Date.now() + Math.random(), rank: teamResults.length + 1, scoreChange: 0, players: ["", ""], fundingRatio: "", fundingAmount: "" }])} className="w-full py-2.5 text-indigo-300 border-2 border-dashed border-indigo-700/50 rounded-lg hover:bg-indigo-900/30 transition font-medium text-sm">새로운 팀 라인 추가</button>
                </div>
              )}
