...
export default function App() {
  ...
  const handleCopyWowApplicantList = () => {
    ...
    try {
      ...
      showToast(
        `버종리 참가 신청 명단(${applicants.split(", ").length}명) 복사 완료!`
      );
    } catch (err) {
  ...
  const handleCopyPubgApplicantList = (formatType) => {
    const applicants = wowRoster
      ...
      .map((m) => {
        ...
        if (formatType === "level_job")
          return `${m.streamerName}(${m.level}렙/${m.jobClass})`;
    ...
    try {
      ...
      showToast(
        `배그 참가 신청 명단(${
          wowRoster.filter((m) => m.isPubgApplied).length
        }명) 복사 완료!`
      );
              ...
              <div className="space-y-6 text-gray-300 leading-relaxed text-sm md:text-base">
                <div>
                  ...
                  <p className="pl-6 text-gray-400">
                    캐릭터들의 레벨 최신화 시점은 화면 최상단 네비게이션 바에
                    표시되는{" "}
                ...
                <div>
                  ...
                  <p className="pl-6 text-gray-400 mb-2 break-keep">
                    ...
                    부득이하게 아래와 같은 방법으로 관리자가 직접 업데이트하고
                    있습니다.
                  </p>
                  <ul className="pl-8 list-decimal text-gray-400 space-y-1.5 marker:font-bold marker:text-blue-400/50">
                    ...
                    <li>
                      우왁굳님 및 참가 스트리머분들의 생방송 화면을 실시간으로
                      모니터링하여 확인
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-gray-900/60 p-5 rounded-lg border border-gray-700/50 mt-4 relative overflow-hidden">
                  ...
                  <p className="text-gray-300 text-base break-keep">
                    ...
                    조금 느리더라도 확실하게, 더 노력하는 관리자가 되겠습니다!
                    감사합니다.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={`border px-3 py-1.5 rounded text-sm font-bold transition flex items-center shadow-sm whitespace-nowrap ${
                      showPubgCopyOptions
                        ? "bg-orange-600 text-white border-orange-500"
                        : "bg-orange-600/20 text-orange-400 border-orange-500/50 hover:bg-orange-600 hover:text-white"
                    }`}
                  >
                    🎯 배그 신청 명단 복사{" "}
                    {showPubgCopyOptions ? (
                            ...
                            <div className="flex flex-col gap-1.5 items-end">
                              {isQualified && (
                                <span className="bg-gradient-to-r from-yellow-600 to-yellow-500 text-black text-[11px] font-bold px-2 py-1 rounded shadow-md flex items-center">
                                  <Ticket className="w-3 h-3 mr-1" /> 버종리
                                  참가권!
                                </span>
                              )}
                              {isPubgQualified && (
                                <span className="bg-gradient-to-r from-gray-800 to-gray-700 text-orange-400 border border-orange-500/50 text-[11px] font-bold px-2 py-1 rounded shadow-md flex items-center">
                                  <Target className="w-3 h-3 mr-1" /> 배그 참가
                                  가능
                                </span>
                              )}
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-8">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            n[idx].fundingAmount =
                              val && totalFunding
                                ? Math.floor(
                                    (Number(totalFunding) * Number(val)) / 100
                                  )
                                : "";
                            setIndividualResults(n);
                          }}
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col gap-2 mr-2">
                      <button
                        onClick={() =>
                          handleToggleWowPartner(member.id, member.isWowPartner)
                        }
                      ...
                      {member.level >= 40 && (
                        <button
                          onClick={() =>
                            handleToggleWowApply(member.id, member.isApplied)
                          }
                          className={`px-3 py-1 rounded text-xs font-bold transition flex items-center border ${
                            member.isApplied
                      ...
                      <button
                        onClick={() =>
                          handleTogglePubgApply(member.id, member.isPubgApplied)
                        }
                        className={`px-3 py-1 rounded text-xs font-bold transition flex items-center border ${
                          member.isPubgApplied
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-lg">
          ...
          <p className="text-sm text-gray-400 mb-6">
            스트리머의 실제 방송국 주소(유튜브, 치지직, SOOP 등)를 입력해주세요.{" "}
            <br />
            (빈칸으로 두시면 선수의 이름으로 자동 검색하여 SOOP으로 연결됩니다.)
          </p>
