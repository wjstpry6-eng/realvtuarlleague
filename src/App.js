...
export default function App() {
  ...
  const buskingParticipants = useMemo(
    () =>
      [...wowRoster]
        .filter(
          (member) => Number(member.level) >= 40 && member.isBuskingParticipant
        )
        .sort((a, b) => {
  ...
  const handleBuskingVote = async (member) => {
    ...
    try {
      await updateDoc(
        doc(db, "artifacts", appId, "public", "data", "wow_roster", member.id),
        { buskingVoteCount: increment(1) }
      );
      const currentRoundId = buskingSettings.roundId || "wow-busking-default";
    ...
    try {
      ...
      const canvas = await html2canvas(target, {
        ...
        onclone: (clonedDocument) => {
          clonedDocument
            .querySelectorAll('[data-no-screenshot="true"]')
            .forEach((element) => {
  ...
  const renderBuskingView = () => {
    const maxVotes = Math.max(
      ...buskingParticipants.map((member) => member.buskingVoteCount || 0),
      0
    );
    const votingStatusLabel = buskingSettings.isVotingOpen
      ? "투표 진행중"
            ...
            <div className="space-y-4 max-w-2xl">
              <div>
                ...
                <p className="text-gray-300 mt-3 break-keep text-sm md:text-base">
                  한 스트리머당 1회만 투표 가능, 여러 스트리머에게는 각각 투표
                  가능
                </p>
              </div>
              <div
                className={`inline-flex items-center px-3 py-1.5 rounded-full border text-sm font-bold ${votingStatusClass}`}
              >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 xl:min-w-[520px]">
              ...
              <div className="bg-white/8 border border-white/10 rounded-2xl p-4">
                ...
                <p className="text-xl font-black text-white truncate">
                  {buskingLeader ? buskingLeader.streamerName : "대기중"}
                </p>
              </div>
              <div className="bg-white/8 border border-white/10 rounded-2xl p-4">
                <p className="text-sm text-gray-300 mb-2">투표 라운드</p>
                <p className="text-lg font-black text-fuchsia-200 truncate">
                  {buskingSettings.roundId || "기본 라운드"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              <h3 className="text-2xl font-black text-white flex items-center">
                <TrendingUp className="w-6 h-6 mr-2 text-rose-400" /> 버스킹
                투표 현황
              </h3>
              <p className="text-sm text-gray-400 mt-1">
                현재 응원 흐름이 가장 뜨거운 스트리머를 한눈에 확인할 수 있어요.
              </p>
            </div>
            <span
              className={`px-3 py-1.5 rounded-full border text-sm font-bold w-fit ${votingStatusClass}`}
            >
              {votingStatusLabel}
            </span>
          </div>
          ...
          {buskingParticipants.length > 0 ? (
            <div className="grid grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)] gap-6">
              <div className="bg-gradient-to-b from-rose-500/15 to-fuchsia-500/10 border border-rose-400/20 rounded-2xl p-5">
                <p className="text-sm text-rose-200 font-bold mb-4">
                  현재 가장 많은 응원을 받는 스트리머
                </p>
                {buskingLeader ? (
                  <div className="flex items-center gap-4">
                    <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-rose-300/60 shadow-[0_0_20px_rgba(244,114,182,0.25)] bg-gray-900">
                      <img
                        ...
                        onError={(e) => {
                          e.target.src = `https://api.dicebear.com/7.x/adventurer/svg?seed=${buskingLeader.streamerName}`;
                        }}
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-2xl font-black text-white truncate">
                        {buskingLeader.streamerName}
                      </p>
                      <p
                        className="text-sm mt-1"
                        style={{
                          color:
                            WOW_CLASS_COLORS[buskingLeader.jobClass] ||
                            "#cbd5e1",
                        }}
                      >
                        {buskingLeader.jobClass} · Lv.{buskingLeader.level}
                      </p>
                      <p className="text-lg font-black text-rose-300 mt-2">
                        {(buskingLeader.buskingVoteCount || 0).toLocaleString()}
                        표
                      </p>
                    </div>
                  </div>
              ...
              <div className="space-y-3">
                {buskingParticipants.map((member, index) => {
                  const percent =
                    maxVotes > 0
                      ? Math.max(
                          ((member.buskingVoteCount || 0) / maxVotes) * 100,
                          8
                        )
                      : 8;
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3 mb-2">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p
                              ...
                              style={{
                                color:
                                  WOW_CLASS_COLORS[member.jobClass] ||
                                  "#94a3b8",
                              }}
                            >
                        ...
                        <p className="text-sm md:text-base font-black text-rose-300">
                          {(member.buskingVoteCount || 0).toLocaleString()}표
                        </p>
                      </div>
              ...
              >
                ...
                <div className="px-4 pb-4 space-y-2">
                  ...
                  <button
                    ...
                    disabled={
                      isProcessing || !buskingSettings.isVotingOpen || hasVoted
                    }
                  ...
                  >
                    {isProcessing ? (
                      <>
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" /> 투표
                        반영중...
                      </>
                    ) : !buskingSettings.isVotingOpen ? (
                      buskingSettings.endedAt ? (
  ...
  const renderRaidView = () => {
    ...
    const raidSlotInfoClass = isUltraDenseRaidLayout
      ? "text-[10px]"
      : "text-[11px]";
    const raidSlotMetaClass = isUltraDenseRaidLayout
      ? "text-[9px]"
      : "text-[10px]";
                        ...
                        <div className={raidGroupInnerClass}>
                          {groupMembers.map((memberId, slotIndex) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`flex items-center gap-2 ${raidSlotPaddingClass} ${raidSlotMinHeightClass}`}
                                >
                                  <div
                                    className={`w-6 shrink-0 ${raidSlotMetaClass} font-black text-center ${
                                      isLocked
                                        ? "text-yellow-300"
                                        : "text-gray-500"
                                    }`}
                                    ...
                                    <>
                                      ...
                                      <div className="min-w-0 flex-1">
                                        ...
                                        >
                                          ...
                                          <span className="font-black text-gray-300 whitespace-nowrap">
                                            Lv. {member.level}
                                          </span>
                                        </div>
                                      </div>
            ...
            <div className="bg-gradient-to-b from-fuchsia-900/20 to-gray-800 rounded-xl p-6 border border-fuchsia-800/40 shadow-lg">
              ...
              <div className="flex flex-wrap gap-3 mb-6">
                <button
                  ...
                  disabled={
                    isBuskingAdminSaving || buskingParticipants.length === 0
                  }
                  className={`px-4 py-2 rounded-lg font-bold text-sm transition ${
                    isBuskingAdminSaving || buskingParticipants.length === 0
                      ? "bg-gray-700 text-gray-400 cursor-not-allowed"
                      : "bg-rose-500 hover:bg-rose-400 text-white shadow-[0_4px_16px_rgba(244,63,94,0.25)]"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-bold text-sm transition ${
                    isBuskingAdminSaving
                      ? "bg-gray-700 text-gray-400 cursor-not-allowed"
                      : "bg-fuchsia-900/50 hover:bg-fuchsia-800 text-fuchsia-200 border border-fuchsia-500/40"
                  }`}
                >
                  투표 초기화
                </button>
                  ...
                  buskingEligibleMembers.map((member) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 flex-wrap md:flex-nowrap">
                        <span className="text-sm font-black text-rose-300 whitespace-nowrap">
                          {(member.buskingVoteCount || 0).toLocaleString()}표
                        </span>
                        <button
                          onClick={() =>
